...
const makeOrmEntity = (): TransactionOrmEntity => {
  ...
  e.id = 'txn-1'; e.reference = 'ref-001'; e.productId = 'p-1';
  e.quantity = 1; e.amountInCents = 100000; e.currency = 'COP';
  e.customerEmail = 'user@test.com'; e.status = TransactionStatus.PENDING;
  e.pspTransactionId = null; e.createdAt = NOW; e.updatedAt = NOW;
  return e;
};

const makeRepo = (overrides = {}) =>
  new TypeOrmTransactionRepository({ findOneBy: jest.fn(), save: jest.fn(), ...overrides } as never);
...
describe('TypeOrmTransactionRepository', () => {
  ...
  describe('save()', () => {
    it('calls repo.save with mapped ORM entity', async () => {
      ...
      await repo.save(txn);
      expect(saveFn).toHaveBeenCalledWith(expect.objectContaining({ id: 'txn-1' }));
    });
  });
});
